# tests/integration/test_e2e_file_metadata_unknown.py
import pytest
from fastmcp import Client
from _util import FIX, require, EXP
from snapshot import assert_snapshot

@pytest.mark.asyncio
async def test_file_metadata_unknown_snapshot():
    p = require(FIX / "misc" / "unknown.bin")
    from keyprobe.server import mcp
    async with Client(mcp) as client:
        res = await client.call_tool("analyze_from_local_path", {"path": str(p)})
        assert_snapshot(res.data, EXP / "file_metadata" / "unknown.json")
