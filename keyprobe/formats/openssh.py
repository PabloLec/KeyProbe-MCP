...
def _parse_public_line(line: bytes) -> Optional[Dict[str, Any]]:
    if not line or not line.startswith((b"ssh-rsa ", b"ssh-ed25519 ", b"ecdsa-sha2-nistp256 ")):
        return None
...
def _parse_private_blob(blob: bytes) -> Dict[str, Any]:
    ...
    priv: Dict[str, Any] = {"cipher": ciphername.decode("utf-8", "replace"), "encrypted": encrypted}
    if kdfname:
        ...
        if kdf == "bcrypt":
            try:
                ...
                priv["kdf_opts"] = {"salt_b64": base64.b64encode(salt).decode("ascii"), "rounds": rounds}
            except Exception:
                pass
    if pub_fp:
...
def _warnings_for_private(priv: Dict[str, Any]) -> List[dict]:
    ...
    if not priv.get("encrypted"):
        warns.append({"code": "OPENSSH_UNENCRYPTED_KEY", "message": "OpenSSH private key is unencrypted", "severity": "warn"})
    if priv.get("kdf") == "bcrypt":
        rounds = int(priv.get("kdf_opts", {}).get("rounds", 0))
        if rounds and rounds < 16:
            warns.append({"code": "OPENSSH_LOW_BCRYPT_ROUNDS", "message": f"bcrypt rounds={rounds} look low", "severity": "warn"})
    return warns
