...
from .common import Warn, days_until, iso_utc
...
def _key_usage(cert: x509.Certificate) -> List[str]:
    ...
    if ku.digital_signature:
        names.append("digitalSignature")
    if ku.content_commitment:
        names.append("contentCommitment")
    if ku.key_encipherment:
        names.append("keyEncipherment")
    if ku.data_encipherment:
        names.append("dataEncipherment")
    if ku.key_agreement:
    ...
    if ku.key_cert_sign:
        names.append("keyCertSign")
    if ku.crl_sign:
        names.append("cRLSign")
    return names
...
def _eku_list(cert: x509.Certificate) -> List[str]:
    ...
    def _eku_name(oid: x509.ObjectIdentifier) -> str:
        if oid == EKUOID.SERVER_AUTH:
            return "serverAuth"
        if oid == EKUOID.CLIENT_AUTH:
            return "clientAuth"
        if oid == EKUOID.CODE_SIGNING:
            return "codeSigning"
        if oid == EKUOID.EMAIL_PROTECTION:
            return "emailProtection"
        if oid == EKUOID.TIME_STAMPING:
            return "timeStamping"
        if oid == EKUOID.OCSP_SIGNING:
            return "OCSPSigning"
    ...
    for ad in aia:
        try:
            if ad.access_method.dotted_string == "1.3.6.1.5.5.7.48.1":  # id-ad-ocsp
                ocsp.append(
                    cast(x509.UniformResourceIdentifier, ad.access_location).value
                )
            elif (
                ad.access_method.dotted_string == "1.3.6.1.5.5.7.48.2"
            ):  # id-ad-caIssuers
                ca_issuers.append(
                    cast(x509.UniformResourceIdentifier, ad.access_location).value
                )
...
def csr_to_meta(csr: x509.CertificateSigningRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        san = [
            g.value if not isinstance(g, x509.IPAddress) else str(g.value)
            for g in cast(x509.SubjectAlternativeName, ext.value)
        ]
    except x509.ExtensionNotFound:
        pass
    ...
    try:
        ...
        ku = []
        if ku_val.digital_signature:
            ku.append("digitalSignature")
        if ku_val.content_commitment:
            ku.append("contentCommitment")
        if ku_val.key_encipherment:
            ku.append("keyEncipherment")
        if ku_val.data_encipherment:
            ku.append("dataEncipherment")
        if ku_val.key_agreement:
            ku.append("keyAgreement")
            if ku_val.encipher_only:
                ku.append("encipherOnly")
            if ku_val.decipher_only:
                ku.append("decipherOnly")
        if ku_val.key_cert_sign:
            ku.append("keyCertSign")
        if ku_val.crl_sign:
            ku.append("cRLSign")
    except x509.ExtensionNotFound:
        pass
    ...
    try:
        ...
        for oid in eku_val:
            ...
            elif oid == EKUOID.CLIENT_AUTH:
                eku.append("clientAuth")
            elif oid == EKUOID.CODE_SIGNING:
                eku.append("codeSigning")
            elif oid == EKUOID.EMAIL_PROTECTION:
                eku.append("emailProtection")
            elif oid == EKUOID.TIME_STAMPING:
                eku.append("timeStamping")
            elif oid == EKUOID.OCSP_SIGNING:
                eku.append("OCSPSigning")
            else:
                eku.append(oid.dotted_string)
    except x509.ExtensionNotFound:
        pass
...
def cert_warnings(meta: Dict[str, Any]) -> List[dict]:
    ...
    try:
        if meta.get("expired"):
            out.append(
                Warn("CERT_EXPIRED", "Certificate is expired", "error").as_dict()
            )
        else:
    ...
    if pk.get("type") == "RSA":
        try:
            if int(pk.get("size", 0)) < 2048:
                out.append(
                    Warn("RSA_WEAK_KEY", "RSA key size < 2048", "warn").as_dict()
                )
        except Exception:
            pass
    ...
    if sig in {"md5", "sha1"}:
        out.append(
            Warn("WEAK_SIGNATURE_HASH", f"Weak signature hash: {sig}", "warn").as_dict()
        )

    if "serverAuth" in meta.get("eku", []) and not meta.get("san"):
        out.append(
            Warn("MISSING_SAN", "serverAuth present but SAN is empty", "warn").as_dict()
        )
    ...
    if bc.get("ca") is True and "keyCertSign" not in meta.get("key_usage", []):
        out.append(
            Warn(
                "CA_MISSING_KU", "CA certificate without keyCertSign", "warn"
            ).as_dict()
        )
